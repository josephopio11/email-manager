      ...
      {accounts?.map((account, index) => {
        return (
          <div key={index} className="flex items-center gap-2">
            ...
            <Keyboard keys={["⌘", (index + 1).toString()]} />
          </div>
        );
      })}
    </div>
...
const Keyboard = ({ keys }: { keys: string[] }) => {
  return (
    <kbd
      className={cn(
        "grid transform-gpu select-none place-content-center text-nowrap rounded-lg px-3 py-1 text-sm font-medium tracking-tighter transition-all duration-100",
...
export default TopAccountSwitcher;
